...
export default function App() {
  ...
  const getMonetary = () => {
    api
      .get(
        '/last/USD-BRL,EUR-BRL,GBP-BRL,JPY-BRL,AUD-BRL,CHF-BRL,CAD-BRL,CNY-BRL,ARS-BRL,TRY-BRL,HKD-BRL,NZD-BRL,SEK-BRL,NOK-BRL,BRL-KRW,SGD-BRL,MXN-BRL,INR-BRL,RUB-BRL,AED-BRL',
      )
      .then(res => {
  ...
  const handlerUpdateInfo = date => {
    ...
    setUpdateInfo(
      ...
      console.log(dt.substring()),
    );
  };

  function calc(moedaEscolhida, simboloMoeda) {
    setSaida(
      `${simboloMoeda}${entrada} = R$ ${(entrada * moedaEscolhida)
        .toFixed(2)
        .replace('.', ',')}`,
    );
